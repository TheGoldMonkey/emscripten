...
local_root = os.path.join(script_dir, '..', '..', 'mcvm', 'system', 'lib', 'libcxxabi')
local_src = os.path.join(local_root, 'src')
local_inc = os.path.join(local_root, 'include')

excludes = ('CMakeLists.txt',)
preserve_files = ('cxa_exception_js_utils.cpp', 'cxa_exception_emscripten.cpp')
...
def main():
  ...
  print(script_dir, emscripten_root, default_llvm_dir, local_root, local_src, local_inc, llvm_dir, upstream_src, upstream_inc)
